/* chinese.js — main logic for the Chinese practice hub (chinese.html).
   Shared pinyin toolkit + esc() live in common.js, loaded first. */
    ...
    // Reading passages for 短文改错, 短文理解 and 听力理解, keyed by lesson id
    // (e.g. "p3-1") regardless of level — see data/README.md.
    let PASSAGE_DATA = {};
    const passageDataPromise = fetch('data/p3-passage.json').then(r => r.json()).catch(() => null)
      .then(d => { PASSAGE_DATA = d || {}; });

    // Pre-synthesized audio for 听力理解 (passage listening comprehension),
    // generated by tools/gen-passage-audio.py from p3-passage.json and keyed
    // the same way: PASSAGE_AUDIO[lessonKey] -> array of
    // { id, theme, whole, segments, sentences }, id matching a PASSAGE_DATA
    // passage's id. Missing until that script has been run with an Azure
    // Speech key, so this degrades to an empty map rather than failing.
    let PASSAGE_AUDIO = {};
    const passageAudioPromise = fetch('passage_audio/manifest.json').then(r => r.json()).catch(() => null)
      .then(d => { PASSAGE_AUDIO = (d && d.lessons) || {}; });
    ...
    function migrateProgress(avatarId, map) {
      ...
      for (const [key, rec] of Object.entries(map)) {
        ...
        if (r.skills && !('listening' in r.skills)) {
          r = { ...r, skills: { ...r.skills, listening: freshSkill() } };
          changed = true;
        }
    ...
    // record so success in one group doesn't hide weakness in another.
    ...
    document.querySelectorAll('#setup-mode-tabs .tab').forEach(tab => {
      tab.addEventListener('click', () => {
        ...
        if (WHOLE_SCREEN_MODES.includes(mode)) {
          S.modes = [mode];
        } else {
          let modes = S.modes.filter(m => !WHOLE_SCREEN_MODES.includes(m));
    ...
    // Turns a passage's question bank into scoreable questions: each
    // question is matched to a character from characters_used (cycled
    // through so 5 questions spread across however many characters the
    // passage uses), with its options pre-shuffled. Returns null if none of
    // the passage's characters_used are in the current lesson's word pool
    // (shouldn't happen for well-formed data, but a session can't score
    // questions against words it can't find).
    function buildPassageQuestions(passage, wordByChar) {
      const usedChars = (passage.characters_used || []).filter(ch => wordByChar.has(ch));
      if (!usedChars.length) return null;
    ...
    async function startPassageMcq() {
      ...
      const pool = getWordPool(S.level, [lessonNum]);
      const wordByChar = new Map(pool.map(w => [w.character, w]));
      const questions = buildPassageQuestions(passage, wordByChar);
      if (!questions) { showToast('Could not link this passage to vocabulary — try again'); return; }
    ...
    // ═══════════════════════════════════════════════════════════════
    // PASSAGE LISTENING MODE (听力理解) — play a passage's pre-synthesized
    // audio (tools/gen-passage-audio.py, chinese/passage_audio/), then answer
    // the same question bank passage-mcq uses. Unlike passage-mcq the
    // passage text is never shown — this tests listening comprehension, not
    // reading — and the question grid stays hidden until the audio has
    // played through once, though it can be replayed any time after that.
    // Scored against the 'listening' skill group, same as 听音.
    // ═══════════════════════════════════════════════════════════════
    let PL = null; // { lessonKey, passage, questions, idx, qStart, audioSrc, hasPlayed }

    async function startPassageListening() {
      await Promise.all([passageDataPromise, passageAudioPromise]);
      const lessonNums = S.lessonTest ? testRandomLessons(S.level) : S.lessons;

      const audioForLesson = n => new Map((PASSAGE_AUDIO[`${S.level}-${n}`] || []).map(a => [a.id, a]));
      const withAudio = lessonNums.filter(n => {
        const audioById = audioForLesson(n);
        return (PASSAGE_DATA[`${S.level}-${n}`] || []).some(p => (p.questions || []).length && audioById.get(p.id)?.whole);
      });
      if (!withAudio.length) { showToast('No listening audio available for this lesson yet'); return; }

      const lessonNum = withAudio[Math.floor(Math.random() * withAudio.length)];
      const lessonKey = `${S.level}-${lessonNum}`;
      const audioById = audioForLesson(lessonNum);
      const passages = PASSAGE_DATA[lessonKey].filter(p => (p.questions || []).length && audioById.get(p.id)?.whole);
      const passage = passages[Math.floor(Math.random() * passages.length)];
      ...
      const audioSrc = `passage_audio/${audioById.get(passage.id).whole}`;
      PL = { lessonKey, passage, questions, idx: 0, qStart: 0, audioSrc, hasPlayed: false };
      showScreen('screen-passage-listening');
      renderPassageListening();
    }
    ...
    document.getElementById('pl-audio').addEventListener('ended', () => {
      if (!PL || PL.hasPlayed) return;
      PL.hasPlayed = true;
      document.getElementById('pl-audio-hint').textContent = 'You can replay the passage anytime — now answer the questions below.';
      document.getElementById('pl-quiz').style.display = 'flex';
      renderPassageListeningQuestion();
    });

    function renderPassageListeningQuestion() {
      const q = PL.questions[PL.idx];
      document.getElementById('pl-progress').textContent = `${PL.idx + 1} / ${PL.questions.length}`;
      document.getElementById('pl-question').textContent = q.question;

      const feedback = document.getElementById('pl-feedback');
      feedback.style.display = 'none';
      feedback.textContent = '';
      document.getElementById('pl-next-row').style.display = 'none';

      const grid = document.getElementById('pl-options-grid');
      grid.innerHTML = '';
      q.shuffledOptions.forEach(opt => {
        const btn = document.createElement('button');
        btn.className = 'opt-btn is-passage-opt';
        btn.textContent = opt;
        btn.addEventListener('click', () => handlePassageListeningAnswer(opt));
        grid.appendChild(btn);
      });
      PL.qStart = Date.now();
    }

    function handlePassageListeningAnswer(chosen) {
      if (!PL) return;
      const q = PL.questions[PL.idx];
      const correct = chosen === q.answer;
      const timeMs = Date.now() - PL.qStart;

      document.querySelectorAll('#pl-options-grid .opt-btn').forEach(btn => {
        if (btn.textContent === q.answer) btn.classList.add(correct ? 'flash-correct' : 'flash-reveal');
        else if (btn.textContent === chosen && !correct) btn.classList.add('flash-wrong');
        btn.disabled = true;
      });
      ...
      const grade = timeMs < 5000 ? 'easy' : timeMs < 10000 ? 'good' : 'hard';
      let rec = S.progress[q.word.key] || freshRecord();
      rec = updateRecord(rec, correct, timeMs, grade, MODE_GROUP['passage-listening']);
      S.progress[q.word.key] = rec;
      saveProgress(S.avatarId, S.progress);
      S.results.push({ word: q.word, correct, timeMs, type: 'passage-listening' });

      const feedback = document.getElementById('pl-feedback');
      feedback.style.display = 'block';
      feedback.textContent = correct ? '✓ Correct!' : `The correct answer was ${q.answer}`;
      document.getElementById('pl-next-row').style.display = 'flex';
    }

    document.getElementById('pl-back').addEventListener('click', () => {
      document.getElementById('pl-audio').pause();
      PL = null;
      showScreen('screen-setup');
    });
