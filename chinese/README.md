...
|------|------------|-------------|
| `chinese.html` | Practice hub: SRS flashcards + ~15 quiz modes across 3 levels, 默写 spelling tests, dictionary browse (with stroke-order + TTS per word), stats, speech (STT/TTS) | `data/chinese-p{1,2,3}.json`, `data/p3-passage.json`, `hanzi-data/`, `pinyin_audio/`, `passage_audio/` |
...
chinese/
  ...
  passage_audio/         听力理解 passage audio, generated by tools/gen-passage-audio.py
                         (see data/README.md) — not committed until an Azure key is run
  hanzi-data/
